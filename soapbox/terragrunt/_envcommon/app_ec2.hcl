...
locals {
  ...
  # VPC Terragrunt path for dependency and dependencies block
  vpc_terragrunt_path = "${get_original_terragrunt_dir()}/../vpc"

  # IAM Terragrunt path for dependency and dependencies block
...
dependency "lookup_iam_module" {
  config_path = "${local.iam_terragrunt_path}"
...
inputs = {
  tags = local.merged_local_vars.ec2_tags
  ec2_instance_profile_name = dependency.lookup_iam_module.outputs.ec2_instance_profile_name
}
...
# The app_ec2 module is dependent on both the vpc and IAM modules completing first
...
# in a Bash script
dependencies {
  paths = ["${local.vpc_terragrunt_path}", "${local.iam_terragrunt_path}"]
}