...
locals {
  ...
  owner            = local.merged_local_vars.owner
  cost_center      = local.merged_local_vars.cost_center
  aws_resource_prefix = local.merged_local_vars.aws_resource_prefix
}
...
remote_state {
  ...
  config = {
    bucket         = "${local.aws_resource_prefix}-tfstate-s3"
